...
def grid_sizer(selected_size, label):
    ...
    number_of_rows = list_of_characters_in_size[0] + (
    list_of_characters_in_size[1]) # Takes first 2 numbers (ex 10 in 10x12)
    number_of_rows = int(number_of_rows)
    number_of_columns = list_of_characters_in_size[3] + (
    list_of_characters_in_size[4]) # Takes last 2 numbers (ex 12 in 10x12)
...
'''Function that exits the starting window; this exit initializes
...
'''Main Function; First Creates Labels in a grid style
 (grid is based off of user input from settings), then every
 300ms checks for a new keyboard input with the 
 check_for_new_key function; Calls update_label
 function to update each label acording
to the last key that has been pressed - This is tracked within the
 update_label function aswell as the 
check_for_new_key function; creates apples function (number of apples
 is based off of user input from settings).'''
def main():
    global window
    global x
    x = 0
    ...
    for current_row in range(number_of_rows): # current row
        for current_column in range(number_of_columns): # current column
            x += 1
            frame = tk.Frame(
                master=window,
                relief=tk.SUNKEN,
            ...
            label = tk.Label(master=frame, text='',
                            width=6, height=3, fg ='white',bg ='#47e322')
             # creates each light green label
            label.pack()
    ...
    for apple in range(apples): #spawns in apples randomly based off of user input
        while True:
            apple_label = squares[f'key{random.randint(
                1, number_of_rows * number_of_columns)}'][0]     
            if (
                apple_label.cget("bg") != 'blue' and
                  apple_label.cget("bg") != 'red'):
                break
...
'''Called within main; physically updates label based off of keyboard
input by calling on_key_press function;
checks if snake runs into itself - if it does then initiates death
screen; creates the apples and checks if
 snake gets an apple - if it does then adds 1 to the length of the
 snake through creating a new
...
def update_label(position):
    ...
    for values in squares.values():
        if values[1] == position[0] and values[2] == position[1]: # if 
            # the current x coordinate in the loop == the current direction
            # and if the current y coordinate in the loop == 
            # the current direction
            # This is for finding the correct label to update after updating
            # the direction.
            label = values[0] # Once the above if statement finds the correct
            # coordinates, it sets value of label = the correct label.
            x_coordinate = values[1]
            y_coordinate = values[2]
            lst = label, x_coordinate, y_coordinate # once above if statement
            # finds correct coordinates, it sets value of lst = label, x, y
            
            if label.cget("bg") == 'blue': #snake dies because it hits itself
                window.destroy()
            elif label.cget("bg") == 'red': #add length when snake gets apple
                length += 1
                while True:
                    apple_label = squares[f'key{random.randint(
                        1, number_of_rows * number_of_columns)}'][0]     
                    if apple_label.cget("bg") != 'blue' and (
                        apple_label.cget("bg") != 'red'):
                        break
                apple_label.config(
                    text='', width=6, height=3, fg='red', bg='red')
                label.config(text='', width=6, height=3, fg='blue', bg='blue')
                window.after(timer * length, delete_label, label)
                 # we can use after the timer to delay the deletion of square
    ...
    if position[0] < 0 or position[0] == number_of_rows:
         # If the snake goes out of the map (the bounds of the grid)
        window.destroy()
...
'''Function that is called from update_label everytime a new key is
 pressed and sets current_key = the key'''
def on_key_press(event):
    global current_key
    current_key = event.keysym
    # bind function returns none, so we made current_key a global 
    # variable instead of returning it


'''Function that is called from main, is constantly checking for a new 
keyboard input update; when it recieves a new
key, checks whether the new key is the opposite key of the former key 
(a is opposite of d, s is opposite of w) in
order to ensure that the snake moves correctly; calls update_label to 
update the future labels to move in the correct direction
until a new key is pressed, then the direction will change based off 
of that key.
'''
def check_for_new_key():
    ...
    if current_key == 'a': # if user presses a
        if former_key == 'd':
    ...
    elif current_key == 'd': # if user presses d
        if former_key == 'a':
            position = position[0], position[1] - 1
            update_label(position)
        else:
    ...
    elif current_key == 'w': # if user presses w
        if former_key == 's':
    ...
    elif current_key == 's': # if user presses s
        if former_key == 'w':
            position = position[0] - 1, position[1]
            update_label(position)
        else:
            ...
            former_key = 's'
            update_label(position)
    else: # If the user presses any key that isn't WASD, it will 
        # just continue to use the former key
        if former_key == 'd':
            position = position[0], position[1] + 1
            update_label(position)
        if former_key == 'a':
    ...
    window.after(timer, check_for_new_key) 
    # Runs itself over and over constantly based on the difficulty 
    # that the user chose.


'''Function that ensures that the length of the snake is correct by 
constantly changing the color of labels
from the color of the snake to the background based off of 
...
def temp_window():
    ...
    temp_win.overrideredirect(True) # Makes it so that the window doesn't 
    # have the bar at the top
    temp_win.focus() # Makes it so that the window is brought infront
    # of the snake grid
    temp_win.attributes('-topmost', True) # Makes it so that the window
    # is brought infront of the snake grid
...
def destroy_temp(temp_win):
    if current_key == 'w' or current_key == 'a' or current_key == 's' or (
    current_key == 'd'):
        temp_win.destroy()
        return
